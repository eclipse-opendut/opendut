...
impl ResourceManager {
    ...
    pub async fn spawn_event_listener<R: Resource + Subscribable>(
        ...
        event_listener::spawn_event_listener(self, on_event).await
    }


    /// Run a closure when two resources are inserted/updated or removed.
    ///
    /// It aggregates the events of these resources, meaning you will
    /// receive a tuple with the most recent event for each.
    ///
    /// The events are wrapped into Options, because when the first event
    /// fires, there is no previous event for the rest.
    pub async fn spawn_event_listener_aggregate_2<R1: Resource + Subscribable, R2: Resource + Subscribable>(
        ...
        event_listener::spawn_event_listener_aggregate_2(self, on_event).await
    }
...
pub mod error {
    pub use super::persistence::error::{PersistenceError, PersistenceResult, MapErrToInner};
