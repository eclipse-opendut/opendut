...
impl ResourceManager {
    ...
    //TODO move implementation into submodule
    pub async fn spawn_event_listener_aggregate_2<R1: Resource + Subscribable, R2: Resource + Subscribable>(
        ...
        let mut subscription1 = self.subscribe::<R1>().await;
        let mut subscription2 = self.subscribe::<R2>().await;
...
mod test {
    ...
    #[tokio::test]
    async fn should_spawn_an_event_listener_that_aggregates_two_resources() -> Result<()> {
        let (testee, _cancel) = ResourceManager::new_in_memory();
