...
pub async fn disconnect_peer_when_removed(resource_manager: ResourceManagerRef, peer_messaging_broker: PeerMessagingBrokerRef) {
    resource_manager.spawn_event_listener::<PeerDescriptor>(async move |event| {
        if let SubscriptionEvent::Removed { id: peer_id, ..  } = event {
...
async fn remove_absent_peer_configuration_parameters_that_are_absent(resource_manager: ResourceManagerRef) {
    ...
    resource_manager.clone().spawn_event_listener::<EdgePeerConfigurationState>(async move |event| {
        if let SubscriptionEvent::Inserted { id, value } = event {
