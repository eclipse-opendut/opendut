    ...
    let handle = tokio::task::spawn_blocking(move || {
        tokio::runtime::Handle::current().block_on(async move {
            let viper_runtime = ViperRuntime::new(ViperOptions {
                source_loaders: vec![Box::new(HttpSourceLoader::default())],
                ..Default::default()
            }).expect("Error while initializing VIPER runtime."); //FIXME introduce error case
            ...
            (compilation.identifier().to_owned(), compilation.parameters().to_owned())
        })
    });

    debug!("VIPER compilation completed.");

    let (identifier, parameters) = handle.await.unwrap(); //TODO don't unwrap?
