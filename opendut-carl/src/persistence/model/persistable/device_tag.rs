...
impl PersistableDeviceTag {
    pub fn insert(&self, connection: &mut PgConnection) -> PersistenceResult<()> {
        diesel::insert_into(schema::device_tag::table)
            ...
            .set(self)
            .execute(connection)
            .map_err(|cause| PersistenceError::insert::<DeviceTag>(self.device_id, cause))?;
        Ok(())
    }
}
