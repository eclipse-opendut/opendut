...
pub mod list {
    ...
    mod test {
        ...
        fn test() {
            ...
            assert_that!(filter_connected_peers(&all_peers, &connected_peers),
...
pub mod describe {
    ...
    pub fn render_peer_descriptor(peer_descriptor: PeerDescriptor, output: DescribeOutputFormat) {
        ...
        let peer_devices = peer_descriptor.topology.devices.iter()
        ...
        let text = match output {
            ...
            DescribeOutputFormat::Json => {
                serde_json::to_string(&peer_descriptor).unwrap()
            }
...
pub mod generate_peer_setup {
    ...
    pub async fn execute(carl: &mut CarlClient, id: Uuid) -> crate::Result<()> {
        ...
        let created_setup = carl.peers.create_peer_setup(peer_id).await
...
pub mod decode_peer_setup {
    ...
    pub async fn execute(setup: PeerSetup, output: DecodePeerSetupOutputFormat) -> crate::Result<()> {
        let text = match output {
            ...
            DecodePeerSetupOutputFormat::Json => {
