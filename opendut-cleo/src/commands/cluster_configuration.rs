...
pub mod create {
    ...
    pub async fn execute(carl: &mut CarlClient, name: String, cluster_id: Option<Uuid>, leader_id: Uuid, device_names: Vec<String>, device_ids: Vec<String>, output: CreateOutputFormat) -> crate::Result<()> {
    ...
    fn check_devices(all_devices: &[DeviceDescriptor], device_names: &[String], device_ids: &[String]) -> Vec<Result<DeviceDescriptor, crate::Error>> {
        ...
        let already_checked_device_names = checked_devices_ids.clone().iter_mut()
            ...
            .map(|test| test.as_mut().unwrap().name.to_string())
            .collect::<Vec<_>>();

        let remaining_device_names= device_names.iter().filter(|device_name| already_checked_device_names.contains(device_name).not()).collect::<Vec<_>>();
    ...
    mod test {
        ...
        #[rstest]
        fn test_check_devices_with_names(all_devices: Vec<DeviceDescriptor>) {
        ...
        #[rstest]
        fn test_that_checked_devices_adds_devices_defined_multiple_times_only_once(all_devices: Vec<DeviceDescriptor>) {
            let device_names = vec![
                String::from("MyDevice"),
                String::from("HisDevice")
            ];
