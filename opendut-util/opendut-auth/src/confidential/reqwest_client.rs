...
impl OidcReqwestClient {
    pub async fn from_config(config: &Config) -> anyhow::Result<reqwest::Client> {
        match Pem::from_config_path(CONFIG_KEY_OIDC_CA, config).await {
            Ok(ca_certificate) => {
                OidcReqwestClient::build_client(ca_certificate)
            }
            Err(_error) => {
    ...
    pub fn from_pem(ca_certificate: Pem) -> anyhow::Result<reqwest::Client> {
        OidcReqwestClient::build_client(ca_certificate)
    }
}
